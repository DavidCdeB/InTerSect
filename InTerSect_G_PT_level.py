...
real_roots_zero_to_four = real_roots[(real_roots >= 0.1) & (real_roots <= 5.0)]
...
np.savetxt('P_H_analytic_intersection_T_%s.dat' %T_folder, output_array_2, header="Temperature (K) \t Pressure_Intersection (GPa) \t H_Intersection = E + PV (a.u.)", fmt="%0.13f")

output_array_3 = np.vstack((T_folder_float, Intersection[0], Intersection[1])).T
np.savetxt('P_H_collisions_intersection_T_%s.dat '%T_folder, output_array_3, header="Temperature (K) \t Pressure_Intersection (GPa) \t H_Intersection = E + PV (a.u.)", fmt="%0.13f")
