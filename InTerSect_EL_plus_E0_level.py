...
plt.ylabel(r'$(H = E + E_{ZP} + PV)$ / F.U. (a.u.)', fontsize=20)
plt.suptitle("PBE-D3, pob-TZVP, SHRINK 8 8, Bipolar 18 18, TOLINTEG 8 18, XXLGRID, TOLDEE 8")
plt.title("(0.87 - 0.98)$V_{eq}$ and (0.98 - 1.08)$V_{eq}$", fontsize=10)
plt.ticklabel_format(useOffset=False)
...
np.savetxt('P_H_analytic_intersection_T_%s_K_plus_ZPE.dat' %T_folder, output_array_2, header="Temperature (K) \t Pressure_Intersection (GPa) \t H_Intersection = E + PV (a.u.)", fmt="%0.13f")

output_array_3 = np.vstack((T_folder, Intersection[0], Intersection[1])).T
np.savetxt('P_H_collisions_intersection_T_%s_K_plus_ZPE.dat' %T_folder, output_array_3, header="Temperature (K) \t Pressure_Intersection (GPa) \t H_Intersection = E + PV (a.u.)", fmt="%0.13f")
